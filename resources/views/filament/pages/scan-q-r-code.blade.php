<x-filament-panels::page>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div class="flex flex-col gap-6">
            <x-filament::section>
                ...
                <x-slot name="description">Arahkan kode qr ke area kamera untuk proses verifikasi otomatis</x-slot>
                ...
                <div class="mt-6 p-4 bg-warning-50 dark:bg-warning-500/10 rounded-xl border border-warning-200 dark:border-warning-500/30">
                    <p class="text-xs font-bold text-warning-700 dark:text-warning-400 mb-2 uppercase tracking-widest flex items-center gap-1">
                        <x-heroicon-o-exclamation-triangle class="w-4 h-4" /> Input Kode Manual
                    </p>
                    <div class="flex gap-2">
                        <input type="text" id="manual-qr-input" placeholder="Tempel kode qr di sini" class="flex-1 rounded-lg border-gray-300 text-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:placeholder-gray-400">
                        <x-filament::button onclick="manualSubmit()" color="warning">Proses</x-filament::button>
                    </div>
                </div>
            </x-filament::section>
        </div>
        ...
        <div class="flex flex-col gap-6">
            <x-filament::section>
                <x-slot name="heading">Informasi Verifikasi</x-slot>
                <x-slot name="description">Hasil pindaian data penerima akan tampil secara otomatis di bawah ini</x-slot>
                ...
                <div class="flex flex-col justify-center min-h-[400px]">
                    ...
                    @if($scanStatus === 'idle')
                        <div class="flex flex-col items-center text-center text-gray-400 dark:text-gray-500">
                            ...
                            <h3 class="text-lg font-bold text-gray-600 dark:text-gray-300">Sistem Siaga</h3>
                            <p class="text-sm mt-1">Kamera terdeteksi aktif silakan pindai kode qr pengguna</p>
                        </div>
                        ...
                        <div class="animate-pulse-once">
                            ...
                            <div class="bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 space-y-5">
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1 font-semibold">Waktu Pindaian</p>
                                        <p class="font-bold text-gray-900 dark:text-gray-100">{{ now()->format('H:i:s WIB') }}</p>
                                    </div>
                                </div>

                                <div class="pt-4 border-t border-gray-200 dark:border-gray-800">
                                    <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-1 font-semibold">Instruksi Operasional</p>
                    ...
                    @elseif($scanStatus === 'error')
                        <div class="animate-pulse-once">
                            ...
                            <div class="bg-danger-50 dark:bg-danger-500/10 rounded-2xl border border-danger-200 dark:border-danger-500/20 p-5 space-y-4">
                                <div>
                                    <p class="text-[10px] text-danger-600 dark:text-danger-400 uppercase tracking-widest mb-1 font-semibold">Alasan Penolakan Sistem</p>
                                ...
                                <div class="pt-4 border-t border-danger-200 dark:border-danger-500/20 grid grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-[10px] text-danger-600 dark:text-danger-400 uppercase tracking-widest mb-1 font-semibold">Identitas Terdeteksi</p>
                                        <p class="font-bold text-danger-900 dark:text-danger-100">{{ $scannedData['name'] }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] text-danger-600 dark:text-danger-400 uppercase tracking-widest mb-1 font-semibold">Kategori Penerima</p>
    ...
    <script>
        ...
        document.addEventListener('livewire:initialized', () => {
            ...
            // Memulai kamera perangkat dengan konfigurasi optimal
            setTimeout(() => {
                html5QrCode.start(
                    { facingMode: "environment" }, 
                    { fps: 10, qrbox: 250, aspectRatio: 1.0 }, 
                    onScanSuccess
                ).catch(err => console.error("Kegagalan akses kamera perangkat ", err));
            }, 500);

            // Mendengarkan respon sukses dari sistem backend untuk melanjutkan pindaian
